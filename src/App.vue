...
export default {
  ...
  watch: {
    ...
    pageStateOptions(value) {
      const currentUrl = `${window.location.pathname}?filter=${value.filter}&page=${value.page}`;
  ...
  computed: {
    ...
    startIndex() {
      return (this.page - 1) * MAX_PAGE_SIZE;
    },

    endIndex() {
      return this.page * MAX_PAGE_SIZE;
    },
  ...
  methods: {
    ...
    getHints(coinList) {
      coinList.sort(
          (coin1, coin2) => coin1.Symbol.length - coin2.Symbol.length
      );
    ...
    isTickerExists(tickerName) {
      let checkedTicker = this.tickers.find(
